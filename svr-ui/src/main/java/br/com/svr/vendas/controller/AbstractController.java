...
public abstract class AbstractController {
	...
	// controllers
	public AbstractController(Result result, UsuarioInfo usuarioInfo, GeradorRelatorioPDF geradorRelatorioPDF, HttpServletRequest request) {
		this.result = result;
		this.request = request;
		this.usuarioInfo = usuarioInfo;
		DIRETORIO_TEMPLATE_PDF = request != null ? request.getServletContext().getRealPath("/templates") + "/" : null;
		...
		try {
			...
			logger.log(Level.SEVERE, "Falha no lookup de algum servico", e);
			this.result.include("erro",
					"Falha no localizacao de algum servico. Verifique o log do servidor para maiores detalhes. CAUSA: " + e.getMessage());
			irTelaErro();
		}
		...
		try {
			...
			logger.log(Level.SEVERE, "O metodo home do controller " + this.getClass().getName() + " nao foi bem definido", e);
			this.result.include("erro", "Falha inicializacao do PATH do metodo home do controller " + this.getClass().getName()
	...
	void addAtributo(String nomeAtributo, Object valorAtributo) {
		result.include(nomeAtributo, valorAtributo);
	}
	...
	void adicionarIdItemSelecionado(Integer[] listaIdItemSelecionado) {
		...
		final Map<Integer, Boolean> idSelecionado = new HashMap<>();
		for (final Integer id : listaIdItemSelecionado) {
	...
	void carregarVendedor(Cliente cliente) {
		...
		if (vendedor == null) {
			/*
			 * Vamos sinalizar o usuario que o cliente que ele pretende efetuar as pesquisas nao possui
			 * vendedor.
			 */
	...
	boolean contemAtributo(String nomeAtributo) {
		return result.included().containsKey(nomeAtributo);
	}
	...
	<T extends AbstractController> T forwardTo(Class<T> classe) {
		if (!result.used()) {
			return result.forwardTo(classe);
		}
	...
	Download gerarDownload(byte[] bytesArquivo, String nomeArquivo, String contentType) {
		return new ByteArrayDownload(bytesArquivo, contentType, StringUtils.removerAcentuacaoESubstituir(nomeArquivo, " ", "_"));
	}
	...
	void gerarListaMensagemAjax(String mensagem, String categoria) {
		...
		lista.add(mensagem);
		result.use(Results.json()).from(lista, categoria).serialize();
	}

	void gerarListaMensagemAlerta(BusinessException e) {
		result.include("listaMensagem", e.getListaMensagem());
		result.include("cssMensagem", cssMensagemAlerta);
	}

	void gerarListaMensagemAlerta(String mensagem) {
		result.include("listaMensagem", new String[] {
				mensagem
		});
		result.include("cssMensagem", cssMensagemAlerta);
	}
	...
	void gerarListaMensagemErro(List<String> listaMensagem) {
		result.include("listaMensagem", listaMensagem);
	...
	void gerarListaMensagemSucesso(Object o, String nomeAtributoExibicao, TipoOperacao tipoOperacao) throws ControllerException {
		Method metodo = null;
		try {
			if (o != null) {
				metodo = o.getClass().getMethod("get" + nomeAtributoExibicao.substring(0, 1).toUpperCase() + nomeAtributoExibicao.substring(1));
				gerarMensagemSucesso(nomeTela + " " + tipoOperacao.getDescricao() + " com sucesso: " + metodo.invoke(o, (Object[]) null));
			}
			else {
				gerarMensagemSucesso(nomeTela + " " + tipoOperacao.getDescricao() + " com sucesso.");
			}

		}
		catch (final Exception e) {
			throw new ControllerException("Não foi possível montar a menagem de sucesso para a inclusao do(a) " + nomeTela);
		}
	}

	void gerarLogErro(String descricaoOperacao) {
		logger.log(Level.SEVERE, "Falha na operacao " + descricaoOperacao);
		result.include("cssMensagem", cssMensagemErro);
		result.include("listaMensagem", new String[] {
				"Falha na operacao " + descricaoOperacao + " .Verifique o log do servidor para maiores detalhes."
		});
		irTelaErro();
	}

	/*
	 * Metodo responsavel por logar ua excecao e redirecionar o usuario para a tela de erro generico.
	 */
	void gerarLogErro(String descricaoOperacao, Exception e) {
		logger.log(Level.SEVERE, "Falha na operacao " + descricaoOperacao, e);
		result.include("cssMensagem", cssMensagemErro);
		result.include("listaMensagem", new String[] {
				"Falha na operacao " + descricaoOperacao + " .Verifique o log do servidor para maiores detalhes. CAUSA: " + e.getMessage()
		});
		irTelaErro();
	}
	...
	/*
	 * Metodo utilizado para efetuar o log da excecao e enviar o "response" via ajax. Como esperamos
	 * que seja uma resposta de uma chamada ajax nao devemos efetuar navegacao alguma tal como,
	 * irTopoPagina, redirect, forward, etc
	 */
	void gerarLogErroRequestAjax(String descricaoOperacao, Exception e) {
		logger.log(Level.SEVERE, "Falha na operacao " + descricaoOperacao, e);
		gerarRetornoErroAjax("Não foi possível inserir o pedido. Veja o log para obter mais informações. " + "Mensagem: " + e.getMessage());
	}
	...
	void gerarMensagemCadastroSucesso(Object o, String nomeAtributoExibicao) throws ControllerException {
		gerarListaMensagemSucesso(o, nomeAtributoExibicao, TipoOperacao.CADASTRO);
	}

	void gerarMensagemRemocaoSucesso() throws ControllerException {
		gerarListaMensagemSucesso(null, null, TipoOperacao.REMOCAO);
	}
	...
	void gerarRetornoAlertaAjax(String mensagem) {
		gerarListaMensagemAjax(mensagem, "alertas");
	}

	void gerarRetornoErroAjax(String mensagem) {
		gerarListaMensagemAjax(mensagem, "erros");
	}
	...
	void habilitarMultiplosLogradouros() {
		result.include(possuiMultiplosLogradouros, true);
	}

	boolean hasAtributo(Object obj) {
		for (final Method metodo : obj.getClass().getMethods()) {
			try {
				if (!"getClass".equals(metodo.getName()) && metodo.getName().startsWith("get") && metodo.invoke(obj, (Object[]) null) != null) {
					return true;
				}
			}
			catch (final Exception e) {
	...
	void inicializarComboTipoLogradouro() {
		result.include("listaTipoLogradouro", tipoLogradouroService.pesquisar());
		result.include("isTipoLogradoutoHabilitado", true);
	}

	/*
	 * Esse metodo ja garante que o usuario sera navegado para o rodape da pagina
	 */
	private void inicializarPaginacao(Integer paginaSelecionada, Integer totalPaginas, Object objetoPaginado, String nomeObjetoPaginado) {
	...
	<T> void inicializarPaginacao(Integer paginaSelecionada, PaginacaoWrapper<T> paginacao, String nomeLista) {
		inicializarPaginacao(paginaSelecionada, calcularTotalPaginas(paginacao.getTotalPaginado()), paginacao.getLista(), nomeLista);
	}

	<T> void inicializarPaginacaoSemRedirecionar(Integer paginaSelecionada, PaginacaoWrapper<T> paginacao, String nomeLista) {
		inicializarPaginacao(paginaSelecionada, calcularTotalPaginas(paginacao.getTotalPaginado()), paginacao.getLista(), nomeLista, false);
	}

	/*
	 * Metodo utilizado para definir qual eh o metodo HOME definido em cada controller, isto eh, o PATH
	 * que aponta para a tela inicial
	 */
	private void inicializarPathHome() throws ControllerException {
		String nome = this.getClass().getSimpleName().replace("Controller", "");
		nome += "Home";
		final Method[] metodos = this.getClass().getDeclaredMethods();
		for (final Method metodo : metodos) {
			if (metodo.getName().equalsIgnoreCase(nome)) {
				final Get get = metodo.getAnnotation(Get.class);
				if (get == null) {
					throw new ControllerException("É obrigatório que o método home " + metodo.getName() + " seja anotado com @Get");
				}
				homePath = "/" + get.value()[0];
				break;
			}
		}
	}

	void inicializarPicklist(String tituloBloco, String tituloElementosNaoAssociados, String tituloElementosAssociados, String nomeAtributoValor,
			String nomeAtributoLabel) {

		this.inicializarPicklist(tituloBloco, tituloElementosNaoAssociados, tituloElementosAssociados, nomeAtributoValor, nomeAtributoLabel, true);
	}

	final void inicializarPicklist(String tituloBloco, String tituloElementosNaoAssociados, String tituloElementosAssociados,
			String nomeAtributoValor, String nomeAtributoLabel, boolean preenchimentoObrigatorio) {

		picklist = new Picklist(result);
		picklist.setTituloBloco(tituloBloco);
		picklist.setTituloElementosNaoAssociados(tituloElementosNaoAssociados);
		picklist.setTituloElementosAssociados(tituloElementosAssociados);
		picklist.setNomeAtributoValor(nomeAtributoValor);
		picklist.setNomeAtributoLabel(nomeAtributoLabel);
		picklist.setPreenchimentoObrigatorio(preenchimentoObrigatorio);
	}

	void inicializarPicklist(String tituloBloco, String tituloElementosNaoAssociados, String tituloElementosAssociados, String nomeAtributoValor,
			String nomeAtributoLabel, boolean preenchimentoObrigatorio, String tituloCampoPesquisa) {
		this.inicializarPicklist(tituloBloco, tituloElementosNaoAssociados, tituloElementosAssociados, nomeAtributoValor, nomeAtributoLabel);
		picklist.setTituloCampoPesquisa(tituloCampoPesquisa);
	}

	void inicializarPicklist(String tituloBloco, String tituloElementosNaoAssociados, String tituloElementosAssociados, String nomeAtributoValor,
			String nomeAtributoLabel, String tituloCampoPesquisa) {
		this.inicializarPicklist(tituloBloco, tituloElementosNaoAssociados, tituloElementosAssociados, nomeAtributoValor, nomeAtributoLabel, true,
				tituloCampoPesquisa);
	}

	<T, K> void inicializarRelatorioPaginado(Integer paginaSelecionada, RelatorioWrapper<T, K> relatorio, String nomeRelatorio) {
		inicializarPaginacao(paginaSelecionada, calcularTotalPaginas((Long) relatorio.getPropriedade("totalPesquisado")), relatorio, nomeRelatorio);
	}

	<T, K> void inicializarRelatorioPaginadoSemRedirecionar(Integer paginaSelecionada, RelatorioWrapper<T, K> relatorio, String nomeRelatorio) {
		inicializarPaginacao(paginaSelecionada, calcularTotalPaginas((Long) relatorio.getPropriedade("totalPesquisado")), relatorio, nomeRelatorio,
				false);
	}

	void init() throws ServiceLocatorException {
		tipoLogradouroService = ServiceLocator.locate(TipoLogradouroService.class);
		usuarioService = ServiceLocator.locate(UsuarioService.class);

		final Field[] listaCampos = this.getClass().getDeclaredFields();
		for (final Field campo : listaCampos) {
	...
	boolean isElementosNaoAssociadosPreenchidosPicklist() {
		return picklist.isElementosNaoAssociadosPreenchidos();
	}
	...
	void loggarFalhaConversaoParametros() {
		if (validator != null && validator.hasErrors()) {
			...
			for (final Message m : validator.getErrors()) {
	...
	void paginarPesquisa(Integer paginaSelecionada, Long totalRegistros) {
		...
		result.include("paginaSelecionada", paginaSelecionada);
		result.include("totalPaginas", calcularTotalPaginas(totalRegistros));
		irRodapePagina();
	}

	void popularPicklist(List<?> elementosNaoAssociados, List<?> elementosAssociados) throws ControllerException {
		picklist.popular(elementosNaoAssociados, elementosAssociados);
	}
	...
	<T extends AbstractController> T redirecTo(Class<T> classe) {
		if (!result.used()) {
			return result.redirectTo(classe);
		}
		return result.of(classe);
	}
	...
	void redirecTo(String path) {
		...
		System.out.println("Recurso: " + path + ". A requisicao ja foi direcionada: " + result.used());
	}
	...
	void serializarJson(SerializacaoJson json) {
		...
		if (json.contemNome()) {
			serializer = result.use(Results.json()).from(json.getObjeto(), json.getNome());
		}
		else {
			serializer = result.use(Results.json()).from(json.getObjeto());
		}
	...
	void verificarPermissaoAcesso(String nomePermissaoAcesso, TipoAcesso... listaTipoAcesso) {
		...
		if (usuarioInfo == null) {
			this.gerarLogErro("acesso a tela " + nomeTela + ", pois o usuario nao foi autenticado pelo sistema. "
					+ "Garanta que a sessao do usuario foi contruida utilizando " + "o contrutor do controller adequado.");
		}
		else if (listaTipoAcesso != null) {
			for (final TipoAcesso tipoAcesso : listaTipoAcesso) {
				if (usuarioInfo.isAcessoPermitido(tipoAcesso)) {
					addAtributo(nomePermissaoAcesso, true);
					break;
				}
			}
		}
...
class Picklist {
	...
	public boolean isElementosAssociadosPreenchidos() {
		return result.included().get(associados) != null;
	}

	public boolean isElementosNaoAssociadosPreenchidos() {
		return result.included().get(naoAssociados) != null;
	}

	void popular(List<?> elementosNaoAssociados, List<?> elementosAssociados) throws ControllerException {

		if (nomeAtributoLabel == null || nomeAtributoValor == null) {
			throw new ControllerException("O nome do atributo para preencher o label e o nome atributo para preencher o valor dos elementos "
		...
		if (elementosNaoAssociados != null) {
			...
			listaElementosNaoAssociados = new ArrayList<PicklistElement>(30);

			for (final Object object : elementosNaoAssociados) {
				try {
					valor = object.getClass().getDeclaredField(nomeAtributoValor);
					label = object.getClass().getDeclaredField(nomeAtributoLabel);

					valor.setAccessible(true);
					label.setAccessible(true);

					listaElementosNaoAssociados.add(new PicklistElement(valor.get(object), label.get(object)));

					valor.setAccessible(false);
					label.setAccessible(false);
				}
				catch (final Exception e) {
					throw new IllegalStateException("Não foi possível montar o picklist da tela para os elementos nao associados", e);
				}
			}
		}
		...
		if (elementosAssociados != null) {
			...
			listaElementosAssociados = new ArrayList<PicklistElement>(30);

			for (final Object object : elementosAssociados) {
				try {
					valor = object.getClass().getDeclaredField(nomeAtributoValor);
					label = object.getClass().getDeclaredField(nomeAtributoLabel);
					...
					listaElementosAssociados.add(new PicklistElement(valor.get(object), label.get(object)));

					valor.setAccessible(false);
					label.setAccessible(false);
				}
				catch (final Exception e) {
					throw new IllegalStateException("Não foi possível montar o picklist da tela para os elementos associados", e);
				}
			}
		}

		result.include(naoAssociados, listaElementosNaoAssociados);
		result.include(associados, listaElementosAssociados);
	}
	...
	public void setPreenchimentoObrigatorio(boolean preenchimentoObrigatorio) {
		result.include("preenchimentoPicklistObrigatorio", preenchimentoObrigatorio);
	}

	public void setTituloBloco(String tituloBloco) {
		result.include("tituloPicklist", tituloBloco);
	}

	public void setTituloCampoPesquisa(String tituloCampoPesquisa) {
		result.include("tituloCampoPesquisa", tituloCampoPesquisa);
		result.include("possuiCampoPesquisa", true);
	}

	public void setTituloElementosAssociados(String tituloElementosAssociados) {
		result.include("labelElementosAssociados", tituloElementosAssociados);
	}

	public void setTituloElementosNaoAssociados(String tituloElementosNaoAssociados) {
		result.include("labelElementosNaoAssociados", tituloElementosNaoAssociados);
	}
}
