...
public class EstoqueController extends AbstractController {
	...
	public void calcularPrecoMinimoItemEstoque(ItemEstoque item) {
		try {
			final Double precoMinimo = estoqueService.calcularPrecoMinimoItemEstoque(item);
			...
			serializarJson(new SerializacaoJson("erros", new String[] {
					"Falha no calculo do preco minimo do item do pedido. Veja o log para mais detalhes."
			}));
	...
	public void calcularValorEstoque(Material material, FormaMaterial formaMaterial) {
		final Double valorEstoque = estoqueService.calcularValorEstoque(material.getId(), formaMaterial);
	...
	public void estoqueHome() {
		...
		verificarPermissaoAcesso("acessoManutencaoEstoquePermitido", TipoAcesso.ADMINISTRACAO, TipoAcesso.MANUTENCAO_ESTOQUE,
	...
	private void gerarRelatorioItemEstoque(List<ItemEstoque> lista) {
		final RelatorioWrapper<String, ItemEstoque> relatorio = new RelatorioWrapper<String, ItemEstoque>("Relatório de itens do estoque");
		String id = null;
		for (final ItemEstoque item : lista) {
	...
	public void inserirConfiguracaoNcmEstoque(Material material, FormaMaterial formaMaterial, String ncm) {
		final boolean configurado = estoqueService.inserirConfiguracaoNcmEstoque(material == null ? null : material.getId(), formaMaterial, ncm);
		if (configurado) {
	...
	public void pesquisarMaterialEstoque(String sigla) {
		...
		if (sigla != null) {
			final List<Material> listaMaterial = materialService.pesquisarBySigla(sigla);
			for (final Material material : listaMaterial) {
	...
	public void pesquisarNcmItemEstoque(ItemEstoque item) {
		try {
			final String ncm = estoqueService.pesquisarNcmItemEstoque(item);
	...
	public void pesquisarPecaByDescricao(String descricao) {
		...
		if (descricao != null && !descricao.isEmpty()) {
			final List<ItemEstoque> listaPeca = estoqueService.pesquisarPecaByDescricao(descricao);
			for (final ItemEstoque peca : listaPeca) {
	...
	public void pesquisarRegistroEstoqueByIdItemEstoque(Integer id, FormaMaterial formaMaterial, Material material, Integer paginaSelecionada) {
		// Alterando a quantidade de registros por pagina
		setNumeroRegistrosPorPagina(100);

		final PaginacaoWrapper<RegistroEstoque> paginacao = registroEstoqueService.paginarRegistroByIdItemEstoque(id,
				calcularIndiceRegistroInicial(paginaSelecionada), getNumeroRegistrosPorPagina());

		for (final RegistroEstoque r : paginacao.getLista()) {
		...
		final ItemEstoque iEst = estoqueService.pesquisarItemEstoqueById(id);
	...
	public void reajustarPrecoItemEstoque(ItemEstoque itemPedido, Material material, FormaMaterial formaMaterial) {
		...
		try {
			...
			if (itemPedido.getId() != null) {
				gerarMensagemSucesso("Item de estoque No. " + itemPedido.getId() + " teve seu preço reajustado com sucesso");
			}
			else {
