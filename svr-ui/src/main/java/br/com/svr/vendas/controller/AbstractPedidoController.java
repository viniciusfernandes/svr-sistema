...
public class AbstractPedidoController extends AbstractController {
	...
	public AbstractPedidoController(Result result, UsuarioInfo usuarioInfo, GeradorRelatorioPDF geradorRelatorioPDF, HttpServletRequest request) {
	...
	Download downloadPDFPedido(Integer idPedido, TipoPedido tipoPedido) {
		try {
			...
			final StringBuilder titulo = new StringBuilder(pedido.isOrcamento() ? "Orcamento " : "Pedido ").append("No. ").append(idPedido)
					.append(" - ").append(pedido.getCliente().getNomeFantasia()).append(".pdf");

			return gerarDownloadPDF(wrapper.getArquivoPDF(), titulo.toString());
		}
		catch (final ConversaoHTML2PDFException e) {
	...
	PedidoPDFWrapper gerarPDF(Integer idPedido, TipoPedido tipoPedido) throws BusinessException {
		...
		if (!isVisulizacaoPermitida(idPedido, tipoPedido)) {
			...
			final Pedido pedido = pedidoService.pesquisarPedidoById(idPedido, TipoPedido.COMPRA.equals(tipoPedido));
			if (pedido == null) {
				throw new BusinessException("Não é possível gerar o PDF do pedido de numero " + idPedido + " pois não existe no sistema.");
			}
			...
			final String template = pedido.isOrcamento() ? "orcamento.html" : "pedido.html";
			final String tipo = pedido.isVenda() ? "Venda" : "Compra";
			...
			if (pedido.isOrcamento()) {
				...
				final List<LogradouroPedido> lLog = pedidoService.pesquisarLogradouro(idPedido);
				if (lLog.isEmpty()) {
					final List<LogradouroCliente> lLogCli = clienteService.pesquisarLogradouroCliente(pedido.getCliente().getId());
					for (final LogradouroCliente l : lLogCli) {
	...
	/*
	 * Metodo dedicado a gerar relatorio paginado dos itens dos pedidos no caso em que o usuario seja
	 * um administrador, sendo assim, ele podera consultar os pedidos de todos os vendedores
	 */
	private RelatorioWrapper<Pedido, ItemPedido> gerarRelatorioPaginadoItemPedido(Integer idCliente, Integer idUsuario, Integer idFornecedor,
			boolean isOrcamento, boolean isCompra, Integer paginaSelecionada, ItemPedido itemVendido) {
		...
		final boolean pesquisarTodos = isAcessoPermitido(ADMINISTRACAO, GERENCIA_VENDAS);
		final RelatorioWrapper<Pedido, ItemPedido> relatorio = relatorioService.gerarRelatorioItemPedidoByIdClienteIdVendedorIdFornecedor(idCliente,
				pesquisarTodos ? null : idUsuario, idFornecedor, isOrcamento, isCompra, indiceRegistroInicial, getNumeroRegistrosPorPagina(),
				itemVendido);

		for (final GrupoWrapper<Pedido, ItemPedido> grupo : relatorio.getListaGrupo()) {
			formatarPedido(grupo.getId());

			for (final ItemPedido itemPedido : grupo.getListaElemento()) {
	...
	void inserirItemPedido(Integer numeroPedido, ItemPedido itemPedido, Double aliquotaIPI) {
		try {
			...
			final Double[] valorPedido = pedidoService.pesquisarValorPedidoByItemPedido(idItemPedido);
			...
			serializarJson(new SerializacaoJson("erros", new String[] {
					"Falha inclusao/alteracao do item do pedido " + numeroPedido + ". Veja o log do servidor para mais detalhes."
			}));
		}
	}
	...
	boolean isPedidoDesabilitado(Pedido pedido) {
		if (pedido == null || isAcessoPermitido(ADMINISTRACAO, GERENCIA_VENDAS)) {
			...
			final SituacaoPedido situacao = pedido.getSituacaoPedido();
			final boolean isCompraFinalizada = pedido.isCompra() && SituacaoPedido.COMPRA_RECEBIDA.equals(situacao);
			final boolean isVendaFinalizada = pedido.isVenda()
					&& (SituacaoPedido.ENVIADO.equals(situacao) || SituacaoPedido.ITEM_AGUARDANDO_COMPRA.equals(situacao)
							|| SituacaoPedido.REVENDA_AGUARDANDO_EMPACOTAMENTO.equals(situacao) || SituacaoPedido.EMPACOTADO.equals(situacao)
							|| SituacaoPedido.COMPRA_ANDAMENTO.equals(situacao) || SituacaoPedido.ITEM_AGUARDANDO_MATERIAL.equals(situacao));
	...
	boolean isVisulizacaoClientePermitida(Integer idCliente) {
		...
		// cliente.
		final boolean isGestor = isAcessoPermitido(ADMINISTRACAO, GERENCIA_VENDAS);
		if (isGestor) {
			return true;
		}
		final Integer idVend = clienteService.pesquisarIdVendedorByIdCliente(idCliente);
		final Integer idUsu = getCodigoUsuario();
		final boolean isAcessoVendaPermitido = (idVend == null || idVend.equals(idUsu)) && isAcessoPermitido(CADASTRO_PEDIDO_VENDAS);
	...
	boolean isVisulizacaoPermitida(Integer idPedido, TipoPedido tipoPedido) {
		final boolean isAdm = isAcessoPermitido(ADMINISTRACAO);
		if (isAdm) {
			return true;
		}
		final boolean isCompra = TipoPedido.COMPRA.equals(tipoPedido);
		final boolean isVenda = !isCompra;
		final boolean isAcessoCompraPermitida = isCompra && isAcessoPermitido(CADASTRO_PEDIDO_COMPRA);
		if (isAcessoCompraPermitida) {
			return true;
		}
		final Integer idCli = pedidoService.pesquisarIdClienteByIdPedido(idPedido);
	...
	void pesquisarContatoClienteByIdCliente(Integer idCliente) {
		final List<ContatoCliente> lContato = clienteService.pesquisarContatoResumidoByIdCliente(idCliente);
		final List<Autocomplete> lista = new ArrayList<>();
		for (final ContatoCliente c : lContato) {
			lista.add(new Autocomplete(c.getId(), c.getNome()));
		}
		serializarJson(new SerializacaoJson("lista", lista));
	}

	void pesquisarPedidoByIdCliente(Integer idCliente, Integer idUsuario, Integer idFornecedor, TipoPedido tipoPedido, boolean orcamento,
			Integer paginaSelecionada, ItemPedido itemVendido, Integer[] listaIdItemSelecionado) {
		final boolean isCompra = TipoPedido.COMPRA.equals(tipoPedido);
		if (idCliente == null) {
			...
			final RelatorioWrapper<Pedido, ItemPedido> relatorio = gerarRelatorioPaginadoItemPedido(idCliente, idUsuario, idFornecedor, orcamento,
					isCompra, paginaSelecionada, itemVendido);
			inicializarRelatorioPaginadoSemRedirecionar(paginaSelecionada, relatorio, "relatorioItemPedido");

			/*
			 * Recuperando os dados do cliente no caso em que nao tenhamos resultado na pesquisa de pedido,
			 * entao os dados do cliente devem permanecer na tela
			 */
			final Cliente cliente = clienteService.pesquisarById(idCliente);
			...
			final LogradouroCliente l = clienteService.pesquisarLogradouroFaturamentoById(idCliente);
			if (l != null) {
	...
	LogradouroPedido recuperarLogradouro(Pedido p, TipoLogradouro t) {
		final List<LogradouroPedido> lLog = p.getListaLogradouro();
		if (t == null || lLog == null || lLog.isEmpty()) {
			return null;
		}
		for (final LogradouroPedido l : lLog) {
	...
	void removerItemPedido(Integer id) {
		try {
			...
			serializarJson(new SerializacaoJson("erros", new String[] {
					"Falha na remoção do item do pedido. Veja o log do servidor para mais detalhes."
			}));
